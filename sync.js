...
async function pullCloud(){try{const res=await fetch(SUPABASE_URL+'/rest/v1/spaceship_state?id=eq.'+encodeURIComponent(STATE_ID)+'&select=data,updated_at',{headers:{'apikey':SUPABASE_KEY,'Authorization':'Bearer '+SUPABASE_KEY}});if(!res.ok)return;const rows=await res.json();if(!rows.length){await pushCloud();return;}const cloud=rows[0].data||{};if(!cloud.tasks)return;syncingFromCloud=true;originalSetItem('spaceshipTasks',JSON.stringify(cloud.tasks));originalSetItem('spaceshipAway',JSON.stringify(cloud.away||null));originalSetItem('spaceshipName',cloud.name||'Captain');syncingFromCloud=false;if(typeof render==='function')render();}catch(e){console.warn('Spaceship sync download failed',e)}}
