function addEditButtons(){
  document.querySelectorAll('.task').forEach(task=>{
    ...
    const match=checkbox.getAttribute('onchange').match(/toggleTask\((\d+)\)/); if(!match)return;
    const id=match[1]; const btn=document.createElement('button'); btn.className='edit-task secondary'; btn.textContent='✏️'; btn.title='Edit task'; btn.style.cssText='padding:8px 10px;flex-shrink:0';
    btn.onclick=(e)=>{e.preventDefault();e.stopPropagation();openEditTask(Number(id));}; const del=task.querySelector('.delete-task'); task.insertBefore(btn,del||null);
  });
}
function openEditTask(id){
  ...
  document.getElementById('newName').value=t.name||'';document.getElementById('newRoom').value=t.room||'Other';document.getElementById('newType').value=t.type||'recurring';document.getElementById('newDays').value=String(t.days||7);document.getElementById('newDecay').value=t.decay||'medium';document.getElementById('newTime').value=t.time||'10 min';toggleFrequency();
...
window.condition=function(t){if(t.type==='once')return t.done?0:100;const p=DECAY[t.decay]||DECAY.medium,a=ageDays(t),raw=100*(1-Math.exp(-Math.log(2)*a/p.half));if(t.decay==='veryfast')return Math.min(100,Math.round(raw));const f=t.days?Math.min(1.6,Math.max(.45,7/t.days)):1;return Math.min(100,Math.round(raw*f));};
